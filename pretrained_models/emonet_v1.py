...
class EmoNet(nn.Module):
    ...
    def forward(self, x, reset_smoothing=False):
        ...
        return {'heatmap': tmp_out, 'expression': final_features[:, :-2], 'valence': final_features[:, -2],
...
class AdaptedEmoNet(nn.Module):
    """
    This is a version of the EmoNet model (Toisoul et al., 2021) that is adapted to fit the regression task of
    predicting continuous valence labels. The last layer of the EmoNet model as well as the forward method were
    adapted accordingly.
    """
    def __init__(self, num_modules=2, n_expression=8, n_reg=2, n_blocks=4, attention=True,
                 temporal_smoothing=False):
        super(AdaptedEmoNet, self).__init__()
        self.num_modules = num_modules
        self.n_expression = n_expression
        self.n_reg = n_reg
        self.attention = attention
    ...
    def forward(self, x, reset_smoothing=False):
        ...
        batch_size = final_features.shape[0]
        final_features = final_features.view(batch_size, final_features.shape[1])
        final_features = self.valence_fc(final_features)
